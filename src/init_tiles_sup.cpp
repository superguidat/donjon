...
static void			init_tiles_sup2(t_prog	*pro,
					    t_zposition	*pos,
					    int32_t	x,
					    int32_t	y)
{
  pro->haut.tiles[(y + pro->height/2) * pro->width+(x + pro->width/2)].
...
void				init_tiles_sup(t_prog	*pro)
{
  ...
  for (int32_t y = -(pro->height/2); y < (pro->height/2); y++)
      ...
      for (int32_t x = -(pro->width/2); x < (pro->width/2); x++)
	{
	  ...
	  init_tiles_sup2(pro, pos, x, y);
	  pro->haut.tiles[(y + pro->height/2) * pro->width+(x + pro->width/2)].setSize(TILE_SIZE);
	}
    }
}
